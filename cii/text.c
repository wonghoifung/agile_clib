...
int Text_find(T s, int i, int j, T str) {
	...
	if (str.len == 0) {
		...
		for (; i < j; i++)
			if (s.str[i] == *str.str) return i + 1;
	} else {
		for (; i + str.len <= j; i++)
			if (equal(s, i, str)) return i + 1;
	}
	return 0;
}
...
int Text_rfind(T s, int i, int j, T str) {
	...
	if (str.len == 0) {
		...
		while (j > i)
			if (s.str[--j] == *str.str) return j + 1;
	} else {
		for (; j - str.len >= i; j--)
			if (equal(s, j - str.len, str)) return j - str.len + 1;
	}
	return 0;
}
...
void test_text() {
	...
	Text_T t1 = Text_put(s1);
	Text_T t2 = Text_put(s2);
	Text_T t3 = Text_put(space);

	Fmt_print("%T\n", &t1);
	Fmt_print("%T\n", &t2);
	Fmt_print("%T\n", &t3);

	Text_T t4 = Text_cat(t1, t3);
	Text_T t5 = Text_cat(t4, t2);
